 ...
 */
async function getSiteProfile(domain, reqUrl) {
  // Prefer an explicit app URL if you set it, otherwise derive from the incoming request.
  ...
  const res = await fetch(`${origin}/api/site/profile`, {
    method: "POST",
...
if (!domain) {
    ...
    const p = (async () => {
      const profile = await getSiteProfile(rootHost, req.url).catch(() => null);
